...
from app.models import Comment, Document, DocumentStatus, DocumentVersion, utcnow
...
async def upload_document(
    ...
    event_bus.emit(
        ...
        actor_id=owner_id,
        payload={"title": document.title, "content": document.content, "version": document.version},
    )
    return document
...
def create_document(payload: DocumentCreate, session: Session = Depends(get_session)):
    ...
    event_bus.emit(
        ...
        entity_id=document.id,
        actor_id=payload.owner_id,
        payload={"title": document.title, "content": document.content, "status": document.status.value},
    )
    return document
...
def list_documents(
    ...
    search: Optional[str] = Query(default=None, description="Case-insensitive title search"),
    session: Session = Depends(get_session),
):
...
def update_document(
    ...
    if payload.status is not None:
        ...
        event_bus.emit(
            ...
            entity_id=document.id,
            actor_id=payload.editor_id,
            payload={"status": document.status.value, "change": "status_only"},
        )
    return document
...
def restore_version(
    ...
    event_bus.emit(
        ...
        payload={"restored_from_version": version_number, "new_version": document.version},
    )
    return document
...
def add_comment(
    ...
    event_bus.emit(
        ...
        actor_id=payload.author_id,
        payload={"document_id": document_id, "text": payload.text},
    )
    return comment
