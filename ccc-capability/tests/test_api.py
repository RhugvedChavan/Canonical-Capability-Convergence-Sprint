...
def test_list_documents_search_by_title(client):
    user = _make_user(client)
    client.post("/documents", json={"title": "The Long Winter", "owner_id": user["id"]})
    client.post("/documents", json={"title": "Summer Notes", "owner_id": user["id"]})
    res = client.get("/documents?search=winter")
    data = res.json()
    assert len(data) == 1
    assert data[0]["title"] == "The Long Winter"
