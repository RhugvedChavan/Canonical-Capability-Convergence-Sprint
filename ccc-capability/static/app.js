...
function applySortOrder() {
  ...
  if (order === "title") {
    state.documents.sort((a, b) => a.title.localeCompare(b.title));
  } else if (order === "status") {
    state.documents.sort((a, b) => a.status.localeCompare(b.status));
  } else {
...
document.getElementById("cleanupBtn").addEventListener("click", async () => {
  const empties = state.documents.filter(
    (d) => (d.title.trim() === "" || d.title.trim().toLowerCase() === "untitled composition") && d.content.trim() === ""
  );
  if (empties.length === 0) {
    alert("No empty untitled drafts to clean up.");
    return;
  }
  if (!confirm(`Delete ${empties.length} empty untitled draft(s)? This can't be undone.`)) return;
...
document.getElementById("newDocBtn").addEventListener("click", async () => {
  ...
  state.originalDoc = null; // already confirmed discard; avoid re-prompting inside selectDocument
...
async function selectDocument(id) {
  ...
  state.originalDoc = { title: doc.title, content: doc.content, status: doc.status };
...
function hasUnsavedChanges() {
  if (!state.originalDoc || !state.selectedDocId) return false;
  return (
    document.getElementById("docTitle").value !== state.originalDoc.title ||
    document.getElementById("docContent").value !== state.originalDoc.content ||
    document.getElementById("docStatus").value !== state.originalDoc.status
  );
}

async function confirmDiscardIfUnsaved() {
  if (!hasUnsavedChanges()) return true;
  return confirm("You have unsaved changes on this manuscript. Discard them and continue?");
}

function updateUnsavedIndicator() {
  document.getElementById("unsavedDot").hidden = !hasUnsavedChanges();
}
...
document.getElementById("docTitle").addEventListener("input", updateUnsavedIndicator);
document.getElementById("docContent").addEventListener("input", () => {
  updateUnsavedIndicator();
  updateEditorStats();
});
document.getElementById("docStatus").addEventListener("change", updateUnsavedIndicator);
...
document.getElementById("saveDocBtn").addEventListener("click", async () => {
  ...
  await loadDocuments();
  state.originalDoc = null; // just saved; skip the discard-check on reselect
...
document.getElementById("duplicateDocBtn").addEventListener("click", async () => {
  const id = state.selectedDocId;
  if (!id) return;
  const title = document.getElementById("docTitle").value;
  const content = document.getElementById("docContent").value;
  const copy = await fetchJSON("/documents", {
    method: "POST",
    body: JSON.stringify({ title: `${title} (copy)`, content, owner_id: state.currentUserId }),
  });
  await loadDocuments();
  state.originalDoc = null;
  await selectDocument(copy.id);
});
...
document.getElementById("exportDocBtn").addEventListener("click", () => {
  ...
  a.href = url;
  a.download = `${title.replace(/[^a-z0-9\-_ ]/gi, "").trim() || "untitled"}.txt`;
  a.click();
